...
class ViewEngine extends hookSystem 
{

     
    public const VIEW_SUFFIX = 'blade';    
	public $config = [
	'views' => homepath.'/bitnameagencyframework/resources/views',
	'cache' => homepath.'/bitnameagencyframework/resources/viewsRender'
	];
    public string $view;
    ...
    public function view(string $viewName, array $data = [], $extends = false)
    {
		...
		$filepath = str_replace(homepath, null, debug_backtrace()[0]['file']).':'.debug_backtrace()[0]['line'];

        $cachePath = $this->config['cache'] . '/' . md5($this->viewName.''.$filepath.''.md5(@implode("|", $data))). '.cache.php';
